...
WA_FLOAT_MARKER = "<!-- nuna-wa-float -->"


def wa_float() -> str:
    """Botao flutuante de WhatsApp (canto inferior direito, plum da marca).
    Auto-contido (markup + estilo). Idempotente via WA_FLOAT_MARKER."""
    return (
        WA_FLOAT_MARKER + "\n"
        '<a href="' + wa_url() + '" class="nuna-wa" target="_blank" '
        'rel="noopener noreferrer" aria-label="Falar no WhatsApp">'
        '<svg viewBox="0 0 32 32" aria-hidden="true" focusable="false" fill="currentColor">'
        '<path d="M16.004 0h-.008C7.174 0 .002 7.174.002 16c0 3.5 1.128 6.745 3.046 9.38L1.05 31.5l6.32-2.02A15.9 15.9 0 0 0 16.004 32C24.83 32 32 24.826 32 16S24.83 0 16.004 0zm9.31 22.6c-.386 1.09-1.92 1.994-3.143 2.258-.836.178-1.928.32-5.604-1.204-4.704-1.95-7.733-6.73-7.97-7.04-.226-.31-1.9-2.53-1.9-4.826 0-2.296 1.166-3.424 1.636-3.904.386-.396.84-.576 1.32-.576.155 0 .295.008.42.014.376.016.566.038.814.632.31.744 1.064 2.6 1.154 2.79.092.19.184.448.06.7-.116.26-.218.382-.41.61-.192.228-.374.404-.566.65-.176.214-.374.444-.152.834.222.382.99 1.632 2.126 2.644 1.464 1.304 2.668 1.71 3.098 1.892.32.134.7.102.934-.146.296-.318.66-.846 1.03-1.366.262-.372.594-.418.94-.286.354.124 2.24 1.056 2.626 1.248.386.192.642.286.736.446.092.16.092.92-.294 2.01z"/>'
        '</svg></a>\n'
        "<style>"
        ".nuna-wa{position:fixed;right:20px;bottom:20px;z-index:9998;width:58px;height:58px;"
        "border-radius:50%;display:flex;align-items:center;justify-content:center;background:#6D5873;"
        "color:#fff;box-shadow:0 8px 24px -6px rgba(50,40,54,.55),0 2px 6px rgba(0,0,0,.18);"
        "text-decoration:none;transition:transform .2s ease,box-shadow .2s ease}"
        ".nuna-wa:hover{transform:translateY(-3px) scale(1.05);"
        "box-shadow:0 14px 32px -8px rgba(50,40,54,.6);color:#fff}"
        ".nuna-wa:active{transform:scale(.97)}"
        ".nuna-wa svg{width:31px;height:31px;display:block}"
        ".nuna-wa::after{content:'';position:absolute;inset:-5px;border-radius:50%;"
        "border:2px solid #6D5873;animation:nuna-wa-pulse 2.6s ease-out infinite;pointer-events:none}"
        "@keyframes nuna-wa-pulse{0%{transform:scale(.82);opacity:.55}100%{transform:scale(1.28);opacity:0}}"
        "@media(max-width:600px){.nuna-wa{width:52px;height:52px;right:16px;bottom:16px}"
        ".nuna-wa svg{width:28px;height:28px}}"
        "@media(prefers-reduced-motion:reduce){.nuna-wa,.nuna-wa::after"
